...
const char *WINDOWTITLE = { "Assignment 3 - Bradford Wong" };
...
Display( )
{
	...
	*/
	...
	if (textureMode == 0) {
		MjbSphere(1.5, 100, 100, Time, false);
	}
		...
		glEnable(GL_TEXTURE_2D);
		//glRotatef(360.*Time, 0., 1., 0.);
		MjbSphere(1.5, 100, 100, Time, true);
		glDisable(GL_TEXTURE_2D);
	}
	...
	//glBindTexture(GL_TEXTURE_2D, "worldtex.bmp");
...
InitMenus( )
{
	...
	glutAddMenuEntry("With Texture", 1);
	glutAddMenuEntry("With Texture and Distortion", 2);
